...
function PureTableRow({
	...
	const cells = columns.map(column => {
		...
		if (cellRenderer) {
			...
			content = dataKey in rowData? rowData[dataKey]: getField(rowData, dataKey);
			if (dataRenderer)
				content = dataRenderer(content);
		}
		return (
			<div
				key={dataKey}
...
function AppTableRow({
	...
	const {rowId, rowData} = React.useMemo(() => {
		...
		const rowData = getRowData? getRowData({rowIndex, rowId, ids, entities}): entities[rowId];
